...
## Function to give order to the market

def give_order(num_positions, check_hours, SYMBOL, VOLUME, direction, sd, SL_SD_BUY, SL_SD_SELL, TP_SD_BUY, TP_SD_SELL) :
    ...
    if (num_positions == 0 and check_hours + check_allowed_trading_hours() == 2) or (num_positions == 0 and not check_hours) :
            ...
            if direction == 'buy' :
                order_result = market_order(SYMBOL, VOLUME, direction, tick.bid - SL_SD_BUY * sd, tick.bid + TP_SD_BUY * sd)
            elif direction == 'sell' :
                order_result = market_order(SYMBOL, VOLUME, direction, tick.bid - SL_SD_SELL * sd, tick.bid + TP_SD_SELL * sd)
            print(order_result)

def give_order_alea(num_positions, check_hours, SYMBOL, VOLUME, direction, sd, SL_SD_BUY, SL_SD_SELL, TP_SD_BUY, TP_SD_SELL, date, alea, random_alea) :
...
def execute_strategy(symbol, volume, timeframe, long_ma_period, rsi_period, auto, directory, file):
    ...
    if initialized:
        ...
        connect_to_account(auto, directory, file)
    ...
    while True:
        ...
        if close > ma_21 and close > ma_50 and close > ma_100:
            if rsi_14 > ma_rsi_14 and fdi < 1.5:
                ...
                give_order(num_positions, check_hours, SYMBOL, VOLUME, direction, sd, SL_SD_BUY, SL_SD_SELL, TP_SD_BUY, TP_SD_SELL)
                ...
                if alea > 0.5 :
                    direction = 'buy'
                    close_positions('sell')
                    random_alea, date = give_order_alea(num_positions, check_hours, SYMBOL, VOLUME, direction, sd, SL_SD_BUY, SL_SD_SELL, TP_SD_BUY, TP_SD_SELL, date, alea, random_alea)
